...
def PowerLawFit(data, data_nature,xmin_rule, *args):
    ...
    if data_nature == 'Continuous':
        if xmin_rule == 'Clauset':
                ...
                fit = pl.Fit(filter(lambda x: x != 0, data), xmin=np.percentile(data, args[1]))
    else:
        if xmin_rule == 'Clauset':
                ...
                fit = pl.Fit(filter(lambda x: x != 0, data), discrete=True, xmin=np.percentile(data, args[1]))
    return fit
...
if approach == 'Static':
        ...
        for i in xrange(1, N, 1):
            ...
            if tail_selected == 'Right' or tail_selected == 'Both':
                ...
                if xmin_rule == 'Clauset':
                    ...
                    fit_1 = PowerLawFit(tail_plus, data_nature,xmin_rule, 'None' ,xmin_sign)
            ...
            if tail_selected == 'Right' or tail_selected == 'Both':
                ...
                fig4.set_title('Log-log plot of the scaling properties of the right-tail for ' + labels[i - 1] + '\n' +'Time Period: ' + dates[0] + ' - ' + dates[-1] + '. Input series: ' + lab)
                fig4.grid()
                fig4.legend()
            ...
            if tail_selected == 'Left' or tail_selected == 'Both':
                ...
                fig4.set_title('Log-log plot of the scaling properties of the left-tail for ' + labels[i - 1] + '\n' + 'Time Period: ' + dates[0] + ' - ' + dates[-1] + '. Input series: ' + lab)
                fig4.grid()
                fig4.legend()
                col_labels = [r'$\hat{\alpha}$', 'Standard err.', r'$x_{min}$', 'size'];
                table_vals = []
                ...
                distribution_list = ['truncated_power_law','exponential','lognormal']
                for pdf in distribution_list:
                    R, p = fit_2.distribution_compare('power law', pdf ,normalized_ratio = True)
        ...
        z.gca().set_position((.1, .20, .83, .70))
        if tail_selected == 'Right' or tail_selected == 'Both':
            z.plot(xrange(1, len(labels) + 1, 1), positive_alpha_vec, marker='^', markersize=10.0, linewidth=0.0,color='green', label='Right tail')
        if tail_selected == 'Left' or tail_selected == 'Both':
            z.plot(xrange(1, len(labels) + 1, 1), negative_alpha_vec, marker='^', markersize=10.0, linewidth=0.0,color='red', label='Left tail')
        ...
        if tail_selected == 'Right' or tail_selected == 'Both':
            ...
            z.gca().set_position((.1, .20, .83, .70))
            z.plot(xrange(1, len(labels) + 1, 1), positive_alpha_vec, marker='o', markersize=7.0, linewidth=0.0,color='green', label='Right tail')
            z.plot(xrange(1, len(labels) + 1, 1), positive_upper_bound, marker='o', markersize=7.0, linewidth=0.0,color='purple', label='Upper bound')
            z.plot(xrange(1, len(labels) + 1, 1), positive_lower_bound, marker='o', markersize=7.0, linewidth=0.0,color='blue', label='Lower bound')
            z.plot(xrange(0, len(labels) + 2, 1), np.repeat(3, len(labels) + 2), color='red')
            z.plot(xrange(0, len(labels) + 2, 1), np.repeat(2, len(labels) + 2), color='red')
            z.xticks(xrange(1, len(labels) + 1, 1), labels)
            z.xlim(xmin=0.5, xmax=len(labels) + 0.5)
            z.ylabel(r'$\alpha$')
        ...
        if tail_selected == 'Left' or tail_selected == 'Both':
            ...
            z.gca().set_position((.1, .20, .83, .70))
            z.plot(xrange(1, len(labels) + 1, 1), negative_alpha_vec, marker='o', markersize=7.0, linewidth=0.0,color='green', label='Left tail')
            z.plot(xrange(1, len(labels) + 1, 1), negative_upper_bound, marker='o', markersize=7.0, linewidth=0.0,color='purple', label='Upper bound')
            z.plot(xrange(1, len(labels) + 1, 1), negative_lower_bound, marker='o', markersize=7.0, linewidth=0.0,color='blue', label='Lower bound')
            z.plot(xrange(0, len(labels) + 2, 1), np.repeat(3, len(labels) + 2), color='red')
            z.plot(xrange(0, len(labels) + 2, 1), np.repeat(2, len(labels) + 2), color='red')
            z.xticks(xrange(1, len(labels) + 1, 1), labels)
            z.xlim(xmin=0.5, xmax=len(labels) + 0.5)
        ...
        amplitude = 0.5
        if tail_selected == 'Right' or tail_selected == 'Both':
            z.bar(np.arange(0, 2 * len(labels), 2), positive_abs_length, amplitude, color='green', label='Right tail')
        if tail_selected == 'Left' or tail_selected == 'Both':
            z.bar(np.arange(amplitude, 2 * len(labels) + amplitude, 2), negative_abs_length, amplitude, color='red',label='Left tail')
        ...
        amplitude = 0.5
        if tail_selected == 'Right' or tail_selected == 'Both':
            z.bar(np.arange(0, 2 * len(labels), 2), positive_rel_length, amplitude, color='green', label='Right tail')
        if tail_selected == 'Left' or tail_selected == 'Both':
            z.bar(np.arange(amplitude, 2 * len(labels) + amplitude, 2), negative_rel_length, amplitude, color='red',label='Left tail')
        z.xticks(np.arange(amplitude, 2 * len(labels) + amplitude, 2), labels)
        ...
        amplitude = 0.5
        if tail_selected == 'Right' or tail_selected == 'Both':
            z.bar(np.arange(0, 2 * len(labels), 2), positive_alpha_KS, amplitude, color='green', label='Right tail')
        if tail_selected == 'Left' or tail_selected == 'Both':
            z.bar(np.arange(amplitude, 2 * len(labels) + amplitude, 2), negative_alpha_KS, amplitude, color='red',label='Left tail')
        z.xticks(np.arange(amplitude, 2 * len(labels) + amplitude, 2), labels)
        z.ylabel('p-value')
        ...
        df = pd.DataFrame({'Input': labels, 'Positive Tail Exponent': matrix_form_transpose[0],
        ...
        for i in xrange(1, N, 1):
            ...
            for l in xrange(initial_index, final_index + 1, an_freq):
                ...
                if plot_storing == 'Yes':
                    ...
                    if tail_selected == 'Right' or tail_selected == 'Both':
                        ...
                        plt.figure('Right tail scaling for ' + labels[i - 1] + begin_date + '_' + end_date)
                        z.gca().set_position((.1, .20, .83, .70))
                        fig4     = fit_1.plot_ccdf(color='b', linewidth=2, label='Empirical CCDF')
                        fit_1.power_law.plot_ccdf(color='b', linestyle='-', label='Fitted CCDF', ax=fig4)
                        fit_1.plot_pdf(color='r', linewidth=2, label='Empirical PDF', ax=fig4)
                    ...
                    if tail_selected == 'Left' or tail_selected == 'Both':
                        ...
                        plt.figure('Left tail scaling for ' + labels[i - 1] + begin_date + '_' + end_date)
                        z.gca().set_position((.1, .20, .83, .70))
                        fig4     = fit_2.plot_ccdf(color='b', linewidth=2, label='Empirical CCDF')
                        fit_2.power_law.plot_ccdf(color='b', linestyle='-', label='Fitted CCDF', ax=fig4)
                        fit_2.plot_pdf(color='r', linewidth=2, label='Empirical PDF', ax=fig4)
            ...
            df = pd.DataFrame({'Date': spec_dates, 'Positive Tail Exponent': matrix_form_transpose[0],
                               'Negative Tail Exponent': matrix_form_transpose[1],
                               'Positive Tail xmin': matrix_form_transpose[2],
                               'Negative Tail xmin': matrix_form_transpose[3],
                               'Positive Tail S.Err': matrix_form_transpose[4],
            ...
            df = df[['Date', 'Positive Tail Exponent', 'Negative Tail Exponent', 'Positive Tail xmin', 'Negative Tail xmin', \
                      'Positive Tail S.Err', 'Negative Tail S.Err', 'Positive Tail Size', 'Negative Tail Size',
